                        ...
                        try:
                            ...
                            try:
                                ...
                                if windll is not None:
                                    windll.kernel32.SetFileAttributesW(str(p), FILE_ATTRIBUTE_NORMAL)
                            except Exception:
                                pass
                        try:
                            func(p)
...
def prepare_local_model_dir(model_name: str, dest_dir: str, trust_remote_code: bool, logger: Any) -> bool:
    ...
    try:
        ...
        try:
            if hasattr(model, "tie_weights") and callable(getattr(model, "tie_weights", None)):
            ...
            elif hasattr(model, "tie_word_embeddings") and callable(getattr(model, "tie_word_embeddings", None)):
                logger.info("Invoking tie_word_embeddings() to canonicalize shared tensors")
                cast(Any, model).tie_word_embeddings()
